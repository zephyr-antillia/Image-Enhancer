...
#
# ImageAdaptiveGausssianThresholder.py
...
#
class ImageAdaptiveGausssianThresholder:
  ...
  def apply(self, img, max_value=255, block_size=11, c=2, image_scaling=3.0): 
    ...
    img = cv2.adaptiveThreshold(img, max_value, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, self.type, block_size, c)
...
if __name__ == "__main__":
  try:
    ...
    thresholder = ImageAdaptiveGausssianThresholder()
    img = thresholder.read(image_file)
    img.show()

    basename = os.path.basename(image_file)
    output_file = os.path.join(output_dir, "gaussian_thresholding_" + basename)
